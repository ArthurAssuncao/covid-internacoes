                ...
                with open(file_path) as arq:
                    ...
                    for data in json_data:
                        if data['territory_name'].lower() == estado.lower():
                            ...
                            data['value'] = float(re.findall(
                                r"([0-9]{1,2}\.[0-9]{2,3})", data['value'])[0])
            ...
            for data in data_completa:
                if data[indice]['epiweek'] == semana:
                    ...
                    if 'valuePerc' in data[indice]:
                        new_value_perc = new_value_perc + \
                            data[indice]['valuePerc']
            estado_data = {
                'epiweek': semana,
        ...
        try:
            for estado in lista_estados:
                if estado not in new_data:
                    new_data[estado] = []
                new_semana_estado = generate_semana_recharts_estados(
                    jsonObj, indice, semana, estado)
                ...
                for estado in regioes_estados[regiao].keys():
                    if not new_data:
                        ...
                        for year_key in year_keys:
                            new_data[year_key] = new_data[
                                year_key] + jsonEstados['estados'][estado][indice][year_key]

                for year_key in year_keys:
                    new_data[year_key] = new_data[year_key] / \
                        len(regioes_estados[regiao].keys())
    ...
    for semana in range(1, max_semanas):
        ...
        try:
            for estado in lista_estados:
                for year_key in year_keys:
                    new_key = '{} ({})'.format(
                        estados_regiao[estado], year_key)
                    if new_key not in semana_data:
                        semana_data['epiweek'] = jsonEstados[
                            'estados'][estado][indice]['epiweek']
                        semana_data['epiweek_label'] = jsonEstados[
                            'estados'][estado][indice]['epiweek_label']
                        semana_data[new_key] = jsonEstados[
                            'estados'][estado][indice][year_key]
                    else:
                        semana_data[new_key] = semana_data[new_key] + \
                            jsonEstados['estados'][estado][indice][year_key]
...
# min_semana = 1
# max_semana = 16
# for semana in range(min_semana, max_semana):
#   success, newJson = download_json(ano, semana)
#   if success:
#     file_path = 'bases_json/base_{}_{}.json'.format(ano, semana)
#     save_file(newJson, file_path)
